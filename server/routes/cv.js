    ...
    try {
        ...
        if (shared.acceptingIMG == true) {
            ...
            if (segregation.includes("paper")) {
                const [insert] = await database.query("INSERT INTO disposal_records (garbage_type, disposal_datetime) VALUES (?, ?)", ["1", new Date()]);
                ...
                const [insert] = await database.query("INSERT INTO disposal_records (garbage_type, disposal_datetime) VALUES (?, ?)", ["4", new Date()]);
                if (insert.warningStatus == 0) {
