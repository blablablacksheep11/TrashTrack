...
#define armVerticalServoPin 32    // 130 degree flat
#define armHorizontalServoPin 22  // Origin: 178 degree, Under camera: 52 degree,
...
void setup() {
  ...
  for (int i = 0; i < 6; i++) {  #include <WiFi.h>
  #include <HTTPClient.h>
  #include <ESP32Servo.h>
  #include <SPI.h>
  #include <MFRC522.h>
  ...
  void loop() {
    ...
    if (sensorState == LOW) {
      if (binstatus) {
        lidServo.write(120);  // Open the bin lid
        if (!opened) {
          closed = false;
