...
void webSocketEvent(WStype_t type, uint8_t *payload, size_t length)
{
  ...
  case WStype_CONNECTED:
    ...
    // You can send a hello message when connected
    webSocket.sendTXT("ESP32 connected");
    break;
  case WStype_TEXT:
    String msg = (char *)payload;
    // Example: 42["updateAnalytics",{"category":"paper"}]

    int eventStart = msg.indexOf("[\"") + 2;
    int eventEnd = msg.indexOf("\"", eventStart);
    String eventName = msg.substring(eventStart, eventEnd);

    if (eventName == "segregation")
    {
      ...
      if (jsonStart > 0)
      {
        ...
        if (deserializeJson(doc, json) == DeserializationError::Ok)
        {
          ...
          else if (strcmp(category, "plastic") == 0)
          {
            categoryID = 2;
          ...
          else if (strcmp(category, "general") == 0)
          {
            categoryID = 3;
...
void setup()
{
  ...
  // NOTE: Socket.IO needs a path "/socket.io/?EIO=4&transport=websocket"
  webSocket.begin("trashtrack-production.up.railway.app", 443, "/socket.io/?EIO=4&transport=websocket");
